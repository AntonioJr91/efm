...
public class ProductTest {
   ...
   void createProduct() {
      Product product = new Product("semente", 100, UnitOfMeasure.UNIT, ProductOrigin.OWN_PRODUCTION, category);
   ...
   void nameIsNull() {
      Assertions.assertThrows(InvalidProductException.class,
              () -> new Product(null, stock, unit, origin, category));
   }
   ...
   void nameIsBlank() {
      Assertions.assertThrows(InvalidProductException.class,
              () -> new Product("   ", stock, unit, origin, category));
   }
   ...
   void nameIsShorter() {
      Assertions.assertThrows(InvalidProductException.class,
              () -> new Product("a", stock, unit, origin, category));
   }
   ...
   void nameIsLonger() {
      Assertions.assertThrows(InvalidProductException.class,
              () -> new Product("a".repeat(51), stock, unit, origin, category));
   }
   ...
   void unitOfMeasureIsNull() {
      Assertions.assertThrows(InvalidProductException.class,
              () -> new Product(name, stock, null, origin, category));
   }

   @Test
   @DisplayName("Should set createdAt when product is created")
   void createdAtValid() {
      Product product = new Product(name, stock, unit, origin, category);
   ...
   void initialStockIsNegative() {
      Assertions.assertThrows(InvalidProductException.class,
              () -> new Product(name, -10, unit, origin, category));
   }
   ...
   void throwErrorWhenCategoryIsNull() {
      Assertions.assertThrows(InvalidProductException.class,
              () -> new Product(name, stock, unit, origin, null));
   }
}
